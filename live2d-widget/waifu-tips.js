...
! function() {
    ...
    class s {
        ...
        async loadModel(t, s, n) {
            if (localStorage.setItem("modelId", t), localStorage.setItem("modelTexturesId", s), o(n, 4e3, 10), this.useCDN) {
                ...
                loadlive2d("live2d", "/live2d-widget/model/model_list.json")
            } else {
                // loadlive2d("live2d", `${this.apiPath}get/?id=${t}-${s}`), console.log(`Live2D 模型 ${t}-${s} 加载完成`)

                // https://live2d.fghrsh.net/api/get/?id=1-53, 请求链接更换模型
                const live2dPathArr = [
                    "model_Pio_school_2017_custume_gray.json",
                    "model_Tia_sakura_costume_navy.json",
                    "model_bilibili_live22.json",
                    "model_shizuku48.json",
                    "model_nepnep.json",
                    "model_noire_classic.json"
                ];
                let num = Math.floor(Math.random() * live2dPathArr.length);
                let filePath = "/live2d-widget/model/" + live2dPathArr[num];
            ...
            callback: () => {
                localStorage.setItem("waifu-display", Date.now()), o("愿你有一天能与重要的人重逢。", 2e3, 11), document.getElementById("waifu")
                    ...
                    document.getElementById("waifu")
                        .style.display = "none", document.getElementById("waifu-toggle")
        ...
        function c(t) {
            ...
            window.addEventListener("mousemove", (() => n = !0)), window.addEventListener("keydown", (() => n = !0)), setInterval((() => {
                ...
                const s = new Date,
                    n = t.split("-")[0],
                    c = t.split("-")[1] || n;
                n.split("/")[0] <= s.getMonth() + 1 && s.getMonth() + 1 <= c.split("/")[0] && n.split("/")[1] <= s.getDate() && s.getDate() <= c.split("/")[1] && (o = (o = e(o))
                ...
                for (let e of t.tools)
                    if (n[e]) {
                        ...
                        document.getElementById("waifu-tool")
                            .insertAdjacentHTML("beforeend", `<span id="waifu-tool-${e}">${t}</span>`), document.getElementById(`waifu-tool-${e}`)
    ...
    window.initWidget = function(e, t) {
        ...
        o.addEventListener("click", (() => {
            o.classList.remove("waifu-toggle-active"), o.getAttribute("first-time") ? (i(e), o.removeAttribute("first-time")) : (localStorage.removeItem("waifu-display"), document.getElementById("waifu")
